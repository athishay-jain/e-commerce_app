...
class CartBloc extends Bloc<CartEvent,CartState>{
  ...
  CartBloc({required this.cartRepo}):super(InitialState()){
    on<AddToCart>((event,emit)async{
      emit(LoadingState());
      try{
        dynamic res = await cartRepo.AddToCart(productId: event.product_id, qty: event.quantity);
        print(res);
        if(res["status"]=="true"){
    ...
    on<GetCart>((event,emit)async{
      ...
      try{
       ...
       if(cart.status){
         print(cart.data);
         emit(LoadedState(cart: cart.data,totalAmount:findTotalAmount(cart: cart.data)));
       }
       else{
    ...
    on<DeleteItem>((event,emit)async{
      ...
      try{
        ...
        if(res["status"]){
          ...
          if(view.status){
            emit(LoadedState(cart: view.data,totalAmount: findTotalAmount(cart: view.data)));
          }else{emit(FailureState(errorMessage: view.message));}
        }else{emit(FailureState(errorMessage: res["message"]));}
      }catch(e){
        emit(FailureState(errorMessage: e.toString()));
      }
    });
